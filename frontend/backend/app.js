...
// get station
  app.get('/api/task/station', (req, res) => {
	const q = 'SELECT * FROM wxyzyqcr_project.bslsntstation';
...
app.post('/api/task/station', async (req, res) => {
	...
	try {
	  // Insert station into the database using a parameterized query to prevent SQL injection
	  await db.query('INSERT INTO wxyzyqcr_project.bslsntstation (station_name) VALUES (?)', [station_name]);
  ...
  app.put('/api/task/station/:selectedStationId', async (req, res) => {
	const { selectedStationId } = req.params;
	const { station_name } = req.body;
  
	try {
	  // Update station name in the database using a parameterized query
	  await db.query('UPDATE wxyzyqcr_project.bslsntstation SET station_name = ? WHERE id = ?', [station_name, selectedStationId]);
  ...
  app.delete('/api/task/station/:selectedStationId', async (req, res) => {
	const { selectedStationId } = req.params;
  
	try {
	  // Delete station from the database using a parameterized query
	  await db.query('DELETE FROM wxyzyqcr_project.bslsntstation WHERE id = ?', [selectedStationId]);
...
// get today
  ...
  app.get('/api/task/today', (req, res) => {
	const q = 'SELECT * FROM wxyzyqcr_project.bslsnttodaytask';
...
app.post('/api/task/today', async (req, res) => {
	...
	try {
	  // Insert todayTask into the database using a parameterized query to prevent SQL injection
	  await db.query('INSERT INTO wxyzyqcr_project.bslsnttodaytask (today_task) VALUES (?)', [todayTask]);
  ...
  app.put('/api/task/today/:selectedStationId', async (req, res) => {
	const { selectedStationId } = req.params;
	const { todayTask : today_task } = req.body;
  
	try {
	  // Update today's task in the database using a parameterized query
	  await db.query('UPDATE wxyzyqcr_project.bslsnttodaytask SET today_task = ? WHERE id = ?', [today_task, selectedStationId]);
  ...
  app.delete('/api/task/today/:selectedStationId', async (req, res) => {
	const { selectedStationId } = req.params;
  
	try {
	  // Delete station from the database using a parameterized query
	  await db.query('DELETE FROM wxyzyqcr_project.bslsnttodaytask WHERE id = ?', [selectedStationId]);
  
	  // Send success response with relevant data
	  res.status(200).json({ message: 'today task deleted successfully' });
	} catch (error) {
	  console.error(error.message);
...
// get tomorrow
  app.get('/api/task/tommorow', (req, res) => {
	const q = 'SELECT * FROM wxyzyqcr_project.bslsnttomorrowplan';
...
app.post('/api/task/tomorrow', async (req, res) => {
	const { tomorrow_plan } = req.body;
  
	try {
	  // Insert tomorrow_plan into the database using a parameterized query to prevent SQL injection
	  await db.query('INSERT INTO wxyzyqcr_project.bslsnttomorrowplan (tomorrow_plan) VALUES (?)', [tomorrow_plan]);
  
	  // Send success response with relevant data
	  res.status(201).json({ message: 'Tomorrow plan added successfully' });
	} catch (error) {
	  console.error(error.message);
	  res.status(500).json({ error: 'Failed to add tomorrow plan' });
	}
  });


  app.put('/api/task/tomorrow/:selectedStationId', async (req, res) => {
	const { selectedStationId } = req.params;
	const { tomorrow_plan } = req.body;
  
	try {
	  // Update tomorrow's plan in the database using a parameterized query
	  await db.query('UPDATE wxyzyqcr_project.bslsnttomorrowplan SET tomorrow_plan = ? WHERE id = ?', [tomorrow_plan, selectedStationId]);
  
	  // Send success response with relevant data
	  res.status(200).json({ message: 'Tomorrow plan updated successfully' });
	} catch (error) {
	  console.error(error.message);
	  res.status(500).json({ error: 'Failed to update tomorrow plan' });
	}
  });


  app.delete('/api/task/tomorrow/:selectedStationId', async (req, res) => {
	const { selectedStationId } = req.params;
  
	try {
	  // Delete station from the database using a parameterized query
	  await db.query('DELETE FROM wxyzyqcr_project.bslsnttomorrowplan WHERE id = ?', [selectedStationId]);
  
	  // Send success response with relevant data
	  res.status(200).json({ message: 'tomorrow plan deleted successfully' });
	} catch (error) {
	  console.error(error.message);
	  res.status(500).json({ error: 'Failed to delete tomorrow plan' });
	}
  });
